...
public class FrostTower : Tower
{
    ...
    // Override the special upgrade effect for Frost Tower
    protected override void ApplySpecialUpgradeEffect()
    {
        // Frost upgrade: Increased slow duration and slightly stronger slow effect
        slowDuration *= 2.0f; // Double the slow duration
        slowAmount *= 1.2f; // 20% stronger slow effect (more slowing)
        
        Debug.Log($"{gameObject.name} upgraded with Extended Frost! New slow duration: {slowDuration}s, Slow amount: {slowAmount}");
    }
} 